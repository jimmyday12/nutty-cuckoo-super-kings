...
econ_fine <- function(overs, econ, field = NULL) {
  ...
  round(floor(econ) - threshold)  + (overs * 2)
  
}

bowling_fines <- bowling_df %>%
  group_by(Name) %>%
  mutate(Economy = econ_fine(Over, Econ),
         `3+` = ifelse(Wkts >= 3, -3, NA)) %>%
  select(User.Id, Name, Economy, `3+`)


fines <- full_join(batting_fines, bowling_fines, by = c("User.Id" = "User.Id", "Name" = "Name"))  %>%
  mutate(Total = sum(`50`, `SR<110`, `SR<160`, `<21`, Duck, Golden, Economy, `3+`, na.rm = TRUE)) 

div(class = "box-score",
  div(class = "box-score-title", "Fines"),
  reactable_function(fines, NULL),
)
```
